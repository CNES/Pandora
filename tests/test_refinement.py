...
class TestRefinement(unittest.TestCase):
    ...
    def test_quadratic(self):
        ...
        gt_mask = np.array([[0, 0, 0, Criteria.PANDORA_MSK_PIXEL_STOPPED_INTERPOLATION]], dtype=np.uint16)
        ...
        np.testing.assert_allclose(
            self.cv["cost_volume"].data, orig_cv["cost_volume"].data, atol=1e10 - 7, rtol=1e10 - 7
        )
    ...
    def test_quadratic_subpix():
        ...
        # Check if the cost volume is not change
        np.testing.assert_allclose(
            subpix_cv["cost_volume"].data, orig_cv["cost_volume"].data, atol=1e10 - 7, rtol=1e10 - 7
        )
    ...
    def test_vfit_with_nan():
        ...
        # Validity mask ground truth
        gt_mask = np.array([[0, Criteria.PANDORA_MSK_PIXEL_STOPPED_INTERPOLATION, 0]], np.uint16)
