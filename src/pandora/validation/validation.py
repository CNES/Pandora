    ...
    def disparity_checking(
        ...
        for row in range(0, nb_row):
            ...
            valid_pixel = np.where(
                (dataset_left["validity_mask"].data[row, :] & cst.Criteria.PANDORA_MSK_PIXEL_INVALID) == 0
            )
            ...
            dataset_left["validity_mask"].data[row, col_left[inside_right][invalid]] += (
                np.uint16(cst.Criteria.PANDORA_MSK_PIXEL_MISMATCH) * comp
            ).astype(np.uint16)
            dataset_left["validity_mask"].data[row, col_left[inside_right][invalid]] -= (
                np.uint16(cst.Criteria.PANDORA_MSK_PIXEL_OCCLUSION) * comp
            ).astype(np.uint16)
