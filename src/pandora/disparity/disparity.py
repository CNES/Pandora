...
class WinnerTakesAll(AbstractDisparity):
    ...
    def to_disp(self, cv: xr.Dataset, img_left: xr.Dataset = None, img_right: xr.Dataset = None) -> xr.Dataset:
        ...
        new_invalid_pixels = np.where(
            np.logical_and(invalid_mc, (disp_map["validity_mask"].data & Criteria.PANDORA_MSK_PIXEL_INVALID) == 0)
        )
        disp_map["validity_mask"].data[new_invalid_pixels] = Criteria.PANDORA_MSK_PIXEL_INVALID
