...
std::tuple<py::array_t<float>, py::array_t<float>> compute_risk(
    ...
    for (int row = 0; row < n_row; ++row) {
        for (int col = 0; col < n_col; ++col) {
            ...
            for (int eta = 0; eta < nbr_etas; ++eta) {
                ...
                sum_for_min += 1 + max_disp - min_disp - r_samp_amb(row, col, eta);
                sum_for_max += max_disp - min_disp;
            }
    ...
    return std::make_tuple(risk_max, risk_min);    
}
...
std::tuple<py::array_t<float>, py::array_t<float>, py::array_t<float>, py::array_t<float>> compute_risk_and_sampled_risk(
    ...
    auto rw_risk_min = risk_min.mutable_unchecked<2>();
    auto rw_risk_max = risk_max.mutable_unchecked<2>();
    py::array_t<float> samp_risk_min = py::array_t<float>({n_row, n_col, nbr_etas});
    py::array_t<float> samp_risk_max = py::array_t<float>({n_row, n_col, nbr_etas});
    auto rw_samp_risk_min = samp_risk_min.mutable_unchecked<3>();
    auto rw_samp_risk_max = samp_risk_max.mutable_unchecked<3>();
    ...
    for (int row = 0; row < n_row; ++row) {
        for (int col = 0; col < n_col; ++col) {
            ...
            for (int eta = 0; eta < nbr_etas; ++eta) {
                ...
                rw_samp_risk_min(row, col, eta) = eta_min_disp;
                rw_samp_risk_max(row, col, eta) = eta_max_disp;
            }
    ...
    return std::make_tuple(risk_max, risk_min, samp_risk_max, samp_risk_min);    
}